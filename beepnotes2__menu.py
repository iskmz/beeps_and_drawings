...
def fn(num):
	os.system("beepnotes2.py "+str(num))
...
# for this specific case, in order NOT to repeat 11 functions with similar calls (except the call number == selected+1)
# a trick was used by calling the same function and modifying "goto_selection()" function's body below (lines 275-283)
my_menu = {
			"1 . Mary Had A Little Lamb":fn,
			"2 . Twinkle Twinkle Little Star":fn,
			"3 . Rock-a-Bye Baby":fn,
			"4 . Happy Birthday to You":fn,
			"5 . If You're Happy and You Know It":fn,
			"6 . We Wish You A Merry Christmas":fn,
			"7 . Ode to Joy":fn,
			"8 . Amazing Grace":fn,
			"9 . Auld Lang Syne (New Year's Eve)":fn,
			"10. Imperial March":fn,
			"11. The Godfather Theme":fn
}
...
def goto_selection():
	if not KEEP_MENU:
		os.system("cls")	
	if selected==len(menu)-1:
		menu_selection[selected]() # exit() command # Quit selection (last item)
	else:
		menu_selection[selected](selected+1) # a trick , specific for this case (check function mapping at the top)
	if not KEEP_MENU:
		exit()
